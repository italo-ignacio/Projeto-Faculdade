...
public class ProtProfC extends JFrame implements ActionListener {
	...
	public void mostrar() {
		...
		int i = 0;
		for (Professor d : dao.mostrarCursos()) {
			dft.insertRow(i,
					new Object[] { d.getId(), d.getIdentidade(), d.getNome(), d.getCursos()});
			i++;
		}
	}
	...
	public void updateActionPerformed(ActionEvent evt) {
		...
		if (table.getSelectedRow() != -1) {
			...
			a.setCursos(table.getValueAt(table.getSelectedRow(), 3).toString()); 

			a.setId(Integer.parseInt(table.getValueAt(table.getSelectedRow(), 0).toString()));

			dao.atualizarCursos(a);
